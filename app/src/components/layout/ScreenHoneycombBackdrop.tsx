...
import {
  ...
  SCREEN_HONEYCOMB_CORNER_BLEED,
} from '../../constants'
import { useTheme } from '../../theme'

export type ScreenHoneycombImageStyle = ImageStyle

type ScreenHoneycombBackdropProps = {
  topLeftImageStyle?: ScreenHoneycombImageStyle
  bottomRightImageStyle?: ScreenHoneycombImageStyle
  centerImageStyle?: ScreenHoneycombImageStyle
  omitCenter?: boolean
}

export function ScreenHoneycombBackdrop({
  topLeftImageStyle,
  bottomRightImageStyle,
  centerImageStyle,
  omitCenter = false,
}: ScreenHoneycombBackdropProps) {
  const { theme } = useTheme()
  const cornerOpacity = theme.mode === 'dark' ? 0.055 : 0.075
  const cornerSecondaryOpacity = theme.mode === 'dark' ? 0.05 : 0.065
  const centerOpacity = theme.mode === 'dark' ? 0.035 : 0.05
